CREATE OR REPLACE VIEW ANAPLAN.TD_GL_SUM AS
WITH
...
CTE_GL_BASE AS (
    ...
    LEFT JOIN CTE_AD_COUNTERUNITS cu
        ON divi = cu.division
       AND ait4 = cu.counterpart
    LEFT JOIN CTE_MD_COSTCENTER cc
        ON cc.l1_division = divi
       AND cc.l2_code = ait2
    WHERE datatype = 'ACT'
      AND acperi >= '202001'
...
CTE_PERIOD_SPINE AS (
    SELECT DISTINCT
        SUBSTRING(cdymd8::TEXT, 1, 6) AS period
    FROM MVXJDTA.CSYCAL
    WHERE SUBSTRING(cdymd8::TEXT, 1, 6)
          BETWEEN '201712' AND TO_CHAR(CURRENT_DATE, 'YYYYMM')
),
CTE_BS_ACCOUNTS AS (
    SELECT
        divi,
...
CTE_BS_MOVEMENTS AS (
    ...
    LEFT JOIN CTE_AD_COUNTERUNITS cu
        ON a.divi = cu.division
       AND a.ait4 = cu.counterpart
    WHERE a.datatype = 'ACT'
    GROUP BY
        a.divi,
        a.ait1,
        a.ait3,
...
CTE_BS_CUMULATIVE AS (
    ...
    FROM CTE_BS_ACCOUNTS f
    CROSS JOIN CTE_PERIOD_SPINE per
    LEFT JOIN CTE_BS_MOVEMENTS dta
        ON f.divi = dta.divi
       AND f.ait1 = dta.ait1
       AND f.ait3 = dta.ait3
       AND f.ait4 = dta.ait4
...
SELECT
    ...
    ad_dim2,
    ad_dim3
FROM CTE_FULL_GL_SUMMARY
WHERE period BETWEEN TO_CHAR(CURRENT_DATE - INTERVAL '2 months', 'YYYYMM')
                 AND TO_CHAR(CURRENT_DATE, 'YYYYMM');

COMMENT ON VIEW ANAPLAN.TD_GL_SUM IS
    'Standalone rolling 3-month GL summary. Inlines AD_COUNTERUNITS and MD_COSTCENTER CTEs. Uses MVXJDTA calendar and bousr sources; compatibility unions point to LIDSKOE/M3SKY.';